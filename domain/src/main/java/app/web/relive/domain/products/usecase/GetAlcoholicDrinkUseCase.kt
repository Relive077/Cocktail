package app.web.relive.domain.products.usecase

import androidx.paging.PagingData
import app.web.relive.domain.base.usecase.GeneralUseCase
import app.web.relive.domain.products.entity.Drink
import app.web.relive.domain.products.repository.DrinkListRepository
import kotlinx.coroutines.flow.Flow
import javax.inject.Inject

class GetAlcoholicDrinkUseCase @Inject constructor(
    private val drinkListRepository: DrinkListRepository,
) : GeneralUseCase<Flow<PagingData<Drink>>, GetAlcoholicDrinkParams> {

    override fun invoke(params: GetAlcoholicDrinkParams): Flow<PagingData<Drink>> =
        drinkListRepository.fetchAlcoholicDrinks()

}

@JvmInline
value class GetAlcoholicDrinkParams(val ids: String)